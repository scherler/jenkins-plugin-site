var msgo = {
  ...
  makeModel:function(data,config){
    // each config should contain an 'attrs' array. Each that has the following options:
    //
    ...
    // default renderers...
    // TODO: THis should go somewhere else... likely with the view for the main grid,
    ...
    function defaultRenerer(attrObj,itemObj,configAttr, type){
    	...
    	var row = itemObj;
    	var customRenderer = (type)?
    			configAttr[type+'Renderer']:
    				configAttr.renderer;
    	...
    	function _renderDisplayObject(val){
    		...
    		$.each(val,function(key,value){
    			html.push(['<div class="objVal ',showKey,'">'].join(''));
    			if(showKey)
    				html.push(['<span class="key ',key,'">',key,'</span>'].join(''));
    			if(!$.isArray(value) && !$.isPlainObject(value))
    				html.push(['<span class="value item end ',key,'">',value,'</span>'].join(''));
    			else
    				html.push(['<span class="value item',key,'">',_renderDisplayObject(value),'</span>'].join(''));
    			html.push('</div>');
    		});
    		return html.join('');
    	};
    	...
    	if($.isFunction(customRenderer)) return customRenderer(attrObj,itemObj,configAttr);
    	if(typeof customRenderer === 'string') return customRenderer;
	...
	function loopGroups(i,group){
		...
		newGroup = $.extend({},group,{items:[],data:group});
	...
	function loopItems(i,item){
		...
		newItem = {attrs:new Array(attrs.length),data:item};
	...
	function loopConfigAttrs(i,configAttr){
		...
		index = i; // needs to be in context for placement of matching attributes...
	...
	function loopDataAttrs(i,dataAttr){
        ...
        // -- group value string or number as determined byt the groupRenderer
        // -- whether or not this item had this attribute
        // -- whatever else was passed into the config...
        

        var newObj = $.extend({val:null,data:null,displayVal:null,sortVal:null,groupVal:null,found:true}, _configAttr);		
        ...
        function _applyRenderers(matcher,configId){
        	...
        	if(matcher === id){
            	newObj.val = dataAttr;
        		newObj.displayVal = defaultRenerer(dataAttr,dataItem,_configAttr);
        		newObj.groupVal = defaultRenerer(dataAttr,dataItem,_configAttr, 'group');
        		newObj.sortVal = defaultRenerer(dataAttr,dataItem,_configAttr, 'sort');
	...
	if(!groupList) startList = itemList;
	if(!itemList && !groupList) startList = data;	
